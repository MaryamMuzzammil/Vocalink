...
const Simulator: React.FC = () => {
  ...
  const startCall = () => {
    setIsCalling(true);
    setTranscript([{ role: 'agent', text: 'Assalam-o-Alaikum! Vocalink Assistant bol raha hoon. Main aapki kya madad kar sakta hoon?' }]);
  };
          ...
          <Card className="p-12 border-none bg-white shadow-[0_40px_80px_-20px_rgba(0,0,0,0.08)] rounded-[4rem] relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col items-center py-10">
              <div className="mb-12 relative">
                ...
                <div className={`w-32 h-32 rounded-full flex items-center justify-center transition-all duration-700 ${isCalling ? 'bg-black shadow-[0_0_50px_rgba(0,0,0,0.2)]' : 'bg-slate-100'}`}>
                  {isCalling ? (
              ...
              <div className="text-center mb-16">
                <h3 className="text-3xl font-medium mb-3 tracking-tight text-slate-900">
                  {isCalling ? 'Agent is Listening...' : 'Start a Voice Session'}
                </h3>
                <div className="flex items-center justify-center gap-3">
                  {isCalling ? (
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
                      <span className="text-lg font-medium text-slate-400 font-mono tracking-widest">{formatTime(callDuration)}</span>
                    </div>
                  ) : (
                    <span className="text-sm font-medium text-slate-300 uppercase tracking-[0.3em]">Ready for simulation</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-8">
                {!isCalling ? (
                  <Button 
                    onClick={startCall}
                    size="lg" 
                    className="h-20 px-16 rounded-[2.5rem] bg-black text-white hover:bg-slate-900 shadow-2xl shadow-black/20 text-xl font-medium uppercase tracking-widest group"
                  >
                  ...
                  <div className="flex gap-4">
                    <Button 
                      ...
                      size="lg" 
                      variant="danger"
                      className="h-20 px-12 rounded-[2.5rem] text-xl font-medium uppercase tracking-widest shadow-2xl shadow-red-500/10"
                    >
                    ...
                    <Button 
                      variant="secondary"
                      size="lg"
                      className="h-20 w-20 rounded-[2.5rem] bg-slate-50 border-none hover:bg-slate-100"
                    >
          ...
          <Card className="h-full flex flex-col border-none bg-white shadow-xl rounded-[4rem] overflow-hidden border border-slate-50">
            ...
            <div className="p-8 border-t border-slate-50 bg-slate-50/30">
              <div className="relative group">
                <input 
                  type="text" 
                  placeholder={isCalling ? "Type a message to simulate user..." : "Start call to simulate..."}
                  disabled={!isCalling}
                  className="w-full bg-white border border-slate-100 rounded-2xl py-4 px-6 text-sm font-medium focus:ring-2 focus:ring-black/5 focus:outline-none transition-all disabled:opacity-50"
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && isCalling) {
                      const input = e.currentTarget;
                      if (input.value) {
                        setTranscript([...transcript, { role: 'user', text: input.value }]);
                        input.value = '';
                      }
                    }
                  }}
